...
class Attention(nn.Module):
    ...
    def dilate_shift(self, x, stride=2):
        _, _, h, w = x.size()
        part1 = x[:, :, ::2, ::2]
        part2 = x[:, :, ::2, 1::2]
        part3 = x[:, :, 1::2, ::2]
        part4 = x[:, :, 1::2, 1::2]
        x = torch.cat((torch.cat((part1, part2), dim=3), torch.cat((part3, part4), dim=3)), dim=2)

        return x

    def dilate_reverse(self, x, stride=2):
        _, _, h, w = x.size()
        out = torch.empty_like(x)
        out[:, :, ::2, ::2] = x[:, :, :int(h / 2), :int(w / 2)]
        out[:, :, ::2, 1::2] = x[:, :, :int(h / 2), int(w / 2):]
        out[:, :, 1::2, ::2] = x[:, :, int(h / 2):, :int(w / 2)]
        out[:, :, 1::2, 1::2] = x[:, :, int(h / 2):, int(w / 2):]
        return out
    ...
    def forward(self, X):
        ...
        if self.use_attn:
            ...
            use_dilate = False
            if use_dilate:
                shifted_QKV = self.dilate_shift(QKV)
            else:
                # shift
            ...
            if use_dilate:
                ...
                out = shifted_out.permute(0, 3, 1, 2)
                attn_out = self.dilate_reverse(out)
            else:
                # merge windows
                shifted_out = window_reverse(attn_windows, self.window_size, Ht, Wt)  # B H' W' C
